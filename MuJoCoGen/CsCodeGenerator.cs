...
namespace MuJoCoGen
{
	...
	{
		private const string Namespace = "Evergine.Bindings.MuJoCo";
		private const string NativeClass = "MuJoCo";
		...
		/// <summary>
		/// True when the element comes from one of the vendored MuJoCo headers. Without this filter
		/// everything reachable from stdlib.h and math.h would be emitted too, since ParseMacros
		/// pulls in the whole preprocessor state.
		/// </summary>
		private static bool IsMuJoCoElement(CppElement element)
		{
			...
			return file.Replace('\\', '/').Contains("/Headers/mujoco/", StringComparison.OrdinalIgnoreCase);
		}
		...
		private void GenerateConstants(CppCompilation compilation, string outputPath)
		{
			...
			{
				if (!IsMuJoCoElement(macro) || macro.Parameters != null)
				{
		...
		private void GenerateEnums(CppCompilation compilation, string outputPath)
		{
			...
			var enums = compilation.Enums
				.Where(e => IsMuJoCoElement(e) && !e.IsAnonymous && e.Items.Count > 0)
				.ToList();
		...
		private void GenerateDelegates(CppCompilation compilation, string outputPath)
		{
			...
			var delegates = compilation.Typedefs
				.Where(t => IsMuJoCoElement(t) && Helpers.ResolvesToFunctionPointer(t, out _))
				.ToList();
		...
		/// <summary>
		/// Input strings are const char* and are marshalled as UTF-8, which is what MuJoCo's XML and
		/// name APIs expect. A non-const char* is a caller-allocated output buffer (mj_loadXML's
		/// error buffer, for instance) and stays a raw byte* so nothing is silently copied back.
		/// </summary>
